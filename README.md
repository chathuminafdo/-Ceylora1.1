...
### Trip Planner
- Shows every destination you've bookmarked. Backed by a MockAPI REST
  resource with local storage as the source of truth, so your trip
  persists across app restarts, survives offline use, and never silently
  loses a change if the API request for it fails.
- Reorder stops by hand with the **▲ / ▼** buttons on each row — your
  custom order is what "add-order" means everywhere else on the screen
  (map, distances, badges).
- An embedded overview map above the list, numbered-pin per stop (matching
  ...
  manual order, or the optimized route once toggled — auto-framed to fit
  every stop.
- **Optimize Route** — reorders your stops using a nearest-neighbour
  ...
  manual order (the ▲/▼ buttons are hidden while an optimized route is
...
- **React Context** ([context/trip.tsx](context/trip.tsx)) owns the trip list's state machine
  ...
  Trip screens without prop drilling. It also fires off the initial cache
  load + API sync and exposes `addToTrip` / `removeFromTrip` / `updateNote`
...
- Reusable UI components: [components/destination-card.tsx](components/destination-card.tsx) (list/featured
  ...
  [components/destination-map.tsx](components/destination-map.tsx) / [trip-map.tsx](components/trip-map.tsx) (embedded maps —
  see **Embedded Maps** below), [components/ui/button.tsx](components/ui/button.tsx) (primary/secondary/tertiary
  button), and [components/app-splash.tsx](components/app-splash.tsx) (launch screen).
...
| Edit a note or reorder | `PUT /tripitems/:id` | Editing a note / ▲▼ buttons in Trip Planner |
...
(`add`/`remove`/`updateNote`/`moveTripItem`) applies to state and
...
- [components/trip-map.tsx](components/trip-map.tsx) — one numbered pin per trip stop (matching the
  ...
  route order, auto-fitted to frame every stop via `fitToCoordinates`.
...
**Note for a production Android build:** `react-native-maps` uses Google
Maps on Android, which Expo Go serves through a shared development key.
An EAS-built standalone APK needs its own key — add
...
a "for development purposes only" watermark. iOS uses Apple Maps and needs
no key.

### REST API Setup
This repo ships with a placeholder MockAPI URL in `lib/config.ts`. To run it
...
2. Add a resource named `tripitems` with fields `destinationId`, `name`,
   `district`, `notes`, `addedAt` (strings), and `order` (number).
3. Copy the resource's base URL (`https://<projectId>.mockapi.io/api/v1`)
   into `MOCKAPI_BASE_URL` in [lib/config.ts](lib/config.ts).
